...
def _scenario():
    return Scenario(
        ...
        target_states=np.array([[10.0,0.0,0.0,2.0],[10.2,0.0,0.0,2.0],[10.4,0.0,0.0,2.0]]),
        obstacles=[],
        reference_speed=2.0,
    )
...
def test_custom_collision_radius_is_used_by_every_planner(monkeypatch):
    records = []
    for name, attr in (("P0","MobilityOnlyPlanner"),("P1","ReactiveConnectivityPlanner"),("P2","PredictiveConnectivityPlanner"),("P3","RiskAwarePredictivePlanner"),("P4","OracleConnectivityPlanner")):
        monkeypatch.setattr(benchmark, attr, lambda *args, _name=name, **kwargs: _RecordingPlanner(_name, records, *args, **kwargs))
    clearance = 3.25
    settings = benchmark.BenchmarkSettings(horizon_steps=3, observation_sigma_m=0.0, p3_mc_samples=1, collision_distance_m=clearance)
    for planner_name in benchmark.PLANNERS:
        benchmark.run_simulated_episode(planner_name, _scenario(), seed=0, settings=settings, link=_DummyLink())
