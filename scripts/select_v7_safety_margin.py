#!/usr/bin/env python3
"""Apply the frozen V7 development-only safety rule to complete margin runs."""
...
def main() -> None:
    ...
    inputs = sorted(args.input_dir.glob("v7_margin_*/episodes.csv"))
    selected = select(inputs, args.output_dir)
    if selected is None:
        raise SystemExit("V7 SAFETY GATE FAILED: no margin selected; confirmation prohibited")
    print(f"FROZEN V7 MARGIN={selected:.1f} m")
