...
def main():
    ...
    ap.add_argument("--input", default="results/real_v2x_replay")
    ...
    # Correct across the complete declared family of replay comparisons. Keep both
    # raw and Holm-adjusted p-values so the paper can distinguish exploratory from
    # multiplicity-controlled inference without silently changing the test family.
    out["holm_p"] = holm_adjust(out["wilcoxon_p"].to_numpy(float))
    out.to_csv(root / "paired_planner_effects.csv", index=False)
