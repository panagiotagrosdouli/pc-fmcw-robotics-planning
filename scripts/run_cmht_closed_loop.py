...
def states_from_xy(xy,dt):
 ...
 if len(xy)>1:v=np.gradient(xy,dt,axis=0);out[:,2]=np.arctan2(v[:,1],v[:,0]);out[:,3]=np.linalg.norm(v,axis=1)
 return out
...
def tracks(rows):
 ...
 for oid,g in df.groupby('object_id',sort=False):
  ...
  for j,(a,b) in enumerate(zip(cuts[:-1],cuts[1:])):
   if b-a>=3:yield f'{oid}:{j}',oid,f[a:b],xy[a:b]
...
def main():
 ...
 for track_id,oid,frames,xy in tracks(rows):
