...
Target motion is measured; ego is simulated; connectivity is modeled. When a
CMHT timestamp file is supplied, CV fitting, target-state kinematics and replay
duration use measured elapsed time. Planner candidate dynamics retain their
configured fixed integration dt, which is reported separately.
"""
...
def initial_ego(hist,dt=None,times=None,gap=12.):
 if times is None:elapsed=dt
 else:elapsed=float(times[-1]-times[-2])
...
def load_transformer(path):
 ...
 ck=torch.load(path,map_location='cpu');model=TrajectoryTransformer(horizon=int(ck['horizon']));model.load_state_dict(ck['state_dict']);model.eval();cal=ck.get('sigma_calibration',{});scale=float(cal.get('scale',1.));source='validation_calibrated' if cal.get('fit_split')=='validation' else 'uncalibrated';return model,int(ck['history']),int(ck['horizon']),scale,source
...
def main():
 ...
 rows=extract_object_positions(a.labels,timestamps=ts);transformer=None;thist=thor=None;tscale=1.;tsource='none'
 if a.transformer_checkpoint:
  transformer,thist,thor,tscale,tsource=load_transformer(a.transformer_checkpoint)
