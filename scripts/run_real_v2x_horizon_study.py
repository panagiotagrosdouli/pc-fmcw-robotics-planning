...
calibration data. Spatial maps are conditioned on known network/direction/speed context.
"""
...
def choose_weight(y,persist,map_pred):
    grid=np.linspace(0,1,41); vals=[np.mean(np.abs(y-(w*persist+(1-w)*map_pred))) for w in grid]; i=int(np.argmin(vals)); return float(grid[i])
...
def main():
    ...
    for h in [int(x) for x in a.horizons.split(",") if x.strip()]:
        ...
        for target,current_col in [("delay_ms","current_delay_ms"),("sinr_db","current_sinr_db")]:
            cy=ca[target].to_numpy(float); cp=ca[current_col].to_numpy(float)
            cglobal=global_maps[target].predict(ca); ccond=cond_maps[target].predict(ca); w=choose_weight(cy,cp,ccond)
            ty=te[target].to_numpy(float); tp=te[current_col].to_numpy(float)
            tglobal=global_maps[target].predict(te); tcond=cond_maps[target].predict(te); tf=w*tp+(1-w)*tcond
            pmae,prmse=err(ty,tp); gmae,grmse=err(ty,tglobal); cmae,crmse=err(ty,tcond); fmae,frmse=err(ty,tf)
            rec={"target":target,"horizon_steps":h,"median_horizon_ms":float(np.median(te.horizon_ms)),"fusion_persistence_weight":w,"persistence_mae":pmae,"global_spatial_mae":gmae,"conditioned_spatial_mae":cmae,"fusion_mae":fmae,"persistence_rmse":prmse,"global_spatial_rmse":grmse,"conditioned_spatial_rmse":crmse,"fusion_rmse":frmse}
            if target=="delay_ms":
                cd=support.evaluate(ca)["nearest_distance_m"]; td=support.evaluate(te)["nearest_distance_m"]; cf=w*cp+(1-w)*ccond
                ...
                rec.update({"global_coverage":float(np.mean((ty>=glo)&(ty<=ghi))),"adaptive_coverage":float(np.mean((ty>=alo)&(ty<=ahi))),"global_width_ms":float(np.mean(ghi-glo)),"adaptive_width_ms":float(np.mean(ahi-alo)),"adaptive_bin_counts":json.dumps(adaptive.counts_),"adaptive_bin_radii_ms":json.dumps(adaptive.radii_.tolist())})
            summary.append(rec)
            temp=te[["run_id"]].copy(); temp["y"]=ty; temp["persist"]=tp; temp["global_map"]=tglobal; temp["cond_map"]=tcond; temp["fusion"]=tf
            for run,g in temp.groupby("run_id"):
                runrows.append({"target":target,"horizon_steps":h,"run_id":run,"n":len(g),"persistence_mae":float(np.mean(np.abs(g["y"]-g["persist"]))),"global_spatial_mae":float(np.mean(np.abs(g["y"]-g["global_map"]))),"conditioned_spatial_mae":float(np.mean(np.abs(g["y"]-g["cond_map"]))),"fusion_mae":float(np.mean(np.abs(g["y"]-g["fusion"]))),"fusion_persistence_weight":w})
