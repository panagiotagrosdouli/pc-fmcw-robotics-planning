#!/usr/bin/env python3
"""Apply the frozen V6 development-only safety rule to complete margin runs."""
...
def select(inputs: list[Path], output_dir: Path) -> float | None:
    ...
    for path in inputs:
        ...
        required = {"seed", "scenario", "planner", "planning_safety_margin_m",
                    ...
                    "margin_relaxation_steps", "min_target_distance_m",
        ...
        rows.append({
            ...
            "margin_relaxation_episodes": int((df.margin_relaxation_steps > 0).sum()),
            "margin_relaxation_steps": int(df.margin_relaxation_steps.sum()),
...
def main() -> None:
    ...
    inputs = sorted(args.input_dir.glob("v6_margin_*/episodes.csv"))
    selected = select(inputs, args.output_dir)
    if selected is None:
        raise SystemExit("V6 SAFETY GATE FAILED: no margin selected; confirmation prohibited")
    print(f"FROZEN V6 MARGIN={selected:.1f} m")
