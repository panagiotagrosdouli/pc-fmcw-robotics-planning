...
def _validate_args(a):
 ...
 if a.dt<=0:raise SystemExit('--dt must be > 0')
 if a.history<2:raise SystemExit('--history must be >= 2')
 if a.horizon<1:raise SystemExit('--horizon must be >= 1')
 if a.observation_sigma_m<0:raise SystemExit('--observation-sigma-m must be >= 0')
 if a.prediction_sigma_m<0:raise SystemExit('--prediction-sigma-m must be >= 0')
 if a.connectivity_weight<0:raise SystemExit('--connectivity-weight must be >= 0')
 if a.mc_samples<1:raise SystemExit('--mc-samples must be >= 1')
 if a.collision_distance_m<0:raise SystemExit('--collision-distance-m must be >= 0')
