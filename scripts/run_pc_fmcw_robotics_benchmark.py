...
def main():
 p=argparse.ArgumentParser();p.add_argument('--seed-start',type=int,default=0);p.add_argument('--seeds',type=int,default=10);p.add_argument('--output-dir',type=Path,default=Path('results/pc_fmcw_sim'));p.add_argument('--dt',type=float,default=.1);p.add_argument('--history',type=int,default=8);p.add_argument('--horizon',type=int,default=20);p.add_argument('--observation-sigma-m',type=float,default=.20);p.add_argument('--prediction-sigma-m',type=float,default=.75);p.add_argument('--connectivity-weight',type=float,default=1.0);p.add_argument('--mc-samples',type=int,default=32);p.add_argument('--collision-distance-m',type=float,default=2.0);p.add_argument('--planning-safety-margin-m',type=float,default=0.0);p.add_argument('--v4-static-viability',action='store_true');p.add_argument('--v5-time-aligned-dynamic',action='store_true');p.add_argument('--v5-dynamic-stop-viability',action='store_true');p.add_argument('--v5-damped-lateral-prediction',action='store_true');p.add_argument('--v6-hierarchical-clearance',action='store_true');p.add_argument('--v7-endpoint-anchored-lateral',action='store_true');a=p.parse_args();_validate_args(a);out=ROOT/a.output_dir;out.mkdir(parents=True,exist_ok=True)
 settings=BenchmarkSettings(dt=a.dt,history_steps=a.history, horizon_steps=a.horizon,observation_sigma_m=a.observation_sigma_m,prediction_sigma_m=a.prediction_sigma_m,connectivity_weight=a.connectivity_weight,p3_mc_samples=a.mc_samples,collision_distance_m=a.collision_distance_m,planning_safety_margin_m=a.planning_safety_margin_m,v4_static_viability=a.v4_static_viability,v5_time_aligned_dynamic=a.v5_time_aligned_dynamic,v5_dynamic_stop_viability=a.v5_dynamic_stop_viability,v5_damped_lateral_prediction=a.v5_damped_lateral_prediction,v6_hierarchical_clearance=a.v6_hierarchical_clearance,v7_endpoint_anchored_lateral=a.v7_endpoint_anchored_lateral);seed_values=range(a.seed_start,a.seed_start+a.seeds);rows=run_benchmark(seeds=seed_values,settings=settings);df=pd.DataFrame(rows);df.to_csv(out/'episodes.csv',index=False)
