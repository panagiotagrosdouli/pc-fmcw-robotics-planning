...
def main():
 ...
 manifest={'study':'dataset-free PC-FMCW-informed closed-loop robotics simulation','seed_start':a.seed_start,'seed_count':a.seeds,'settings':settings.__dict__,'planners':['P0','P1','P2','P3','P4'],'target_motion':'simulated','ego_motion':'simulated receding-horizon control','prediction':'P2/P3 share constant-velocity mean; P4 alone uses future truth','link_provenance':PCFMCWPlanningLinkPredictor().provenance(),'claim_boundary':'Simulation study; not optical measurements and not a real-vehicle trial.'};(out/'manifest.json').write_text(json.dumps(manifest,indent=2),encoding='utf-8');print(summary.to_string(index=False))
