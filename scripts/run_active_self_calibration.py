...
def _seed_values(config,phase,seed_start=None,seeds=None):
    if phase=="smoke":
        start=0 if seed_start is None else int(seed_start);count=1 if seeds is None else int(seeds)
    ...
    if seed_start is not None or seeds is not None:
        start=int(lo if seed_start is None else seed_start);count=int(len(declared) if seeds is None else seeds)
        if count<1:raise SystemExit("--seeds must be >= 1")
        return list(range(start,start+count))
    return declared
...
def _hyperparameters(config,phase,args):
    ...
    if phase=="confirmatory":
        if args.frozen_protocol is None:raise SystemExit("confirmatory execution requires --frozen-protocol")
...
def _settings(config,hyper):
    ...
    return CalibrationBenchmarkSettings(
        ...
        connectivity_weight=float(s["connectivity_weight"]),information_weight=float(hyper["information_weight"]),
        probe_weight=float(hyper["probe_weight"]),decision_threshold=float(hyper["decision_threshold"]),
        min_expected_regret=float(hyper.get("min_expected_regret",0.0)),
        information_horizon_steps=int(s["information_horizon_steps"]),collision_distance_m=float(s["collision_distance_m"]),
        planning_safety_margin_m=float(s["planning_safety_margin_m"]),
        candidate_lateral_offsets=tuple(s["candidate_lateral_offsets"]),candidate_horizons=tuple(s["candidate_horizons"]),
        ...
        bounded_brake_steer=bool(s["bounded_brake_steer"]),time_aligned_dynamic=bool(s["time_aligned_dynamic"]),
...
def _one_run(config,args,hyper,seed_values,link_model,setting_id=None,planners=None):
    ...
    episodes["link_geometry_model"]=args.link_model;steps["link_geometry_model"]=args.link_model
    for key,value in hyper.items():episodes[key]=value;steps[key]=value
    return settings,episodes,steps

def _write_outputs(out,episodes,steps,manifest):
    out.mkdir(parents=True,exist_ok=True);episodes.to_csv(out/"episodes.csv",index=False);steps.to_csv(out/"steps.csv",index=False)
    summary=episodes.groupby("planner",as_index=False).agg(
        episodes=("seed","size"),collision_rate=("collision_indicator","mean"),
        mean_regret=("cumulative_decision_regret","mean"),mean_probe_fraction=("probe_fraction","mean"),
        mean_parameter_error=("final_parameter_error_normalized","mean"),mean_progress_m=("progress_m","mean"),
        mean_outage=("mean_outage_probability","mean"),
    )
    summary.to_csv(out/"summary.csv",index=False);(out/"manifest.json").write_text(json.dumps(manifest,indent=2),encoding="utf-8")
...
def main():
    ...
    p.add_argument("--output-dir",type=Path,default=None);p.add_argument("--seed-start",type=int,default=None)
    p.add_argument("--seeds",type=int,default=None);p.add_argument("--scenario-steps",type=int,default=None)
    p.add_argument("--decision-threshold",type=float,default=None);p.add_argument("--information-weight",type=float,default=None)
    ...
    if args.sweep_development_grid and phase!="development":raise SystemExit("--sweep-development-grid is valid only for development")
    ...
    frozen=None;link_model=ParameterizedPCFMCWLinkModel(directional=args.link_model=="directional")
    if args.sweep_development_grid:
        grid=config["development"]["hyperparameter_grid"];episodes_all=[];steps_all=[];setting_records=[];settings=None
        for idx,values in enumerate(itertools.product(grid["decision_threshold"],grid["information_weight"],grid["probe_weight"])):
    ...
    if phase!="smoke" and (args.seed_start is not None or args.seeds is not None):evidence_role="subset_or_exploratory_run_not_full_declared_evidence"
    manifest={
        "schema_version":1,"protocol_version":config["protocol_version"],"phase":phase,"evidence_role":evidence_role,
        "created_utc":datetime.now(timezone.utc).isoformat(),"git_commit_sha":_git_sha(),"config_path":str(args.config),
        ...
        "link_provenance":link_model.provenance(),
        "software":{"python":platform.python_version(),"platform":platform.platform(),"numpy":np.__version__,"pandas":pd.__version__},
