...
class LatentLinkParameters:
    ...
    def __post_init__(self) -> None:
        ...
        if not np.all(np.isfinite(values)):
            raise ValueError("latent link parameters must be finite")
        if self.alpha_loss <= 0.0:
            raise ValueError("alpha_loss must be positive")
        if self.k_angular <= 0.0:
            raise ValueError("k_angular must be positive")
...
class ParameterizedPCFMCWLinkModel:
    """PC-FMCW-informed analytical link model with modeled latent mismatch."""
    ...
    def predict(
        ...
        ego_trajectory,
        target_prediction,
        parameters: LatentLinkParameters | None = None,
        link_history=None,
    ) -> LinkForecast:
        del link_history
        phi = parameters or NOMINAL_LATENT_PARAMETERS
        ego = self._states(ego_trajectory)
        ...
        distance_loss_db = (
            ...
            * g.pathloss_exponent
            * phi.alpha_loss
            * np.log10(distance / g.reference_distance_m)
        )
        angular_gain = np.exp(
            -0.5 * phi.k_angular * (angle_error / max(g.beam_sigma_rad, 1e-12)) ** 2
        )
