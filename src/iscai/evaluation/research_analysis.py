...
All confirmatory planner effects are paired by scenario and seed. Candidate-level
counts and receding-horizon steps are never treated as independent inferential
samples.
"""
...
DEFAULT_METRICS = (
    ...
    "min_static_obstacle_clearance_m",
    "collision_indicator",
    "min_realized_ttc_s",
    "no_candidate_rate",
)
        ...
        for baseline, candidate in comparisons:
            for metric in metrics:
                ...
                rows.append({
                    ...
                    "n_pairs": int(len(paired)), "n_seed_clusters": int(clustered["n_clusters"]),
...
def scenario_paired_effects(frame: pd.DataFrame, comparisons=DEFAULT_COMPARISONS,
                            metrics=DEFAULT_METRICS, *, bootstrap_samples=10000, rng=2026) -> pd.DataFrame:
    rows = []
    for scenario, group in frame.groupby("scenario", sort=False):
        effects = paired_effects(group, comparisons=comparisons, metrics=metrics,
                                 bootstrap_samples=bootstrap_samples, rng=rng)
...
def value_of_information(frame: pd.DataFrame, metric="mean_outage_probability") -> pd.DataFrame:
    ...
    subset = frame[(frame["planner"].isin(["P1", "P2", "P3", "P4"])) & (frame["experiment"] == "core")]
    if subset.empty:
        return pd.DataFrame()
    return subset.groupby(["setting_id", "planner"], as_index=False).agg(mean=(metric, "mean"), std=(metric, "std"), n=(metric, "size"))
...
def reliability_mobility_pareto(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    grouped = frame.groupby(["experiment", "setting_id", "planner"], as_index=False).agg(outage=("mean_outage_probability", "mean"), progress=("progress_m", "mean"))
    result = []
    for experiment, group in grouped.groupby("experiment", sort=False):
        ...
        group = group.copy(); group["pareto_efficient"] = pareto_mask(values, minimize=[True, False]); result.append(group)
...
def compute_frontier(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    grouped = frame.groupby(["experiment", "setting_id", "planner"], as_index=False).agg(outage=("mean_outage_probability", "mean"), planning_time_s=("mean_planning_time_s", "mean"), p95_planning_time_s=("p95_planning_time_s", "mean"))
    result = []
    for experiment, group in grouped.groupby("experiment", sort=False):
        values = group[["outage", "planning_time_s"]].to_numpy(float)
        group = group.copy(); group["pareto_efficient"] = pareto_mask(values, minimize=[True, True]); result.append(group)
    return pd.concat(result, ignore_index=True) if result else pd.DataFrame()
...
def prediction_break_even(effects: pd.DataFrame, metric="mean_outage_probability") -> pd.DataFrame:
    if effects.empty:
        return pd.DataFrame()
    subset = effects[(effects["experiment"].isin(["shift", "blackout"])) & (effects["baseline"] == "P1") & (effects["candidate"] == "P2") & (effects["metric"] == metric)].copy()
    if subset.empty:
        return subset
...
def feasibility_diagnostics(frame: pd.DataFrame) -> pd.DataFrame:
    columns = ["candidate_evaluations", "candidate_road_rejections", "candidate_speed_rejections",
    ...
    for keys, group in frame.groupby(["experiment", "setting_id", "scenario", "planner"], sort=False):
        total = float(group["candidate_evaluations"].sum()); steps = float(group["steps"].sum())
        row = dict(zip(["experiment", "setting_id", "scenario", "planner"], keys)); row["candidate_evaluations"] = int(total)
        for column, label in (("candidate_road_rejections", "road_rejection_fraction"), ("candidate_speed_rejections", "speed_rejection_fraction"), ("candidate_static_rejections", "static_rejection_fraction"), ("candidate_dynamic_rejections", "dynamic_rejection_fraction"), ("candidate_feasible", "feasible_candidate_fraction")):
