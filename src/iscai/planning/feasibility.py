...
def check_dynamic_target(states: np.ndarray, target_xy: np.ndarray, min_clearance: float = 2.0) -> bool:
    ...
    ``states`` and ``target_xy`` are expected to describe the same future time
    indices. Candidate trajectories generated by :mod:`trajectory` include the
    current ego state at index 0, so callers must pass ``candidate.states[1:]``
    when the target prediction starts at the next planning step.
    """
...
def filter_dynamic_target(candidates, target_xy, min_clearance=2.0):
    ...
    feasible = []
    for candidate in candidates:
        future_states = candidate.states[1:]
        ok = check_dynamic_target(future_states, target_xy, min_clearance)
...
def filter_with_diagnostics(candidates, target_xy=None, obstacles=None,
    ...
    for candidate in candidates:
        if not check_road_bounds(candidate.states, lane_half_width):
            ...
            candidate.states[1:], target_xy, target_clearance
        ):
            counts["dynamic"] += 1
        else:
            counts["feasible"] += 1
