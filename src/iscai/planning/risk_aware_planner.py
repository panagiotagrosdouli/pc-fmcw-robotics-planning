...
class RiskAwarePredictivePlanner(_BasePlanner):
    ...
    ``target_prediction`` must be a dict with keys ``mean_xy`` and ``sigma_xy``
    of shape (H, 2). Monte-Carlo target trajectories are propagated through the
    same link-predictor interface used by P1/P2, avoiding a second inconsistent
    channel model inside the planner.
    """
    ...
    def plan(self, ego_state, target_prediction, obstacles=None, reference_speed=None):
        ...
        for idx, candidate in enumerate(candidates):
            ...
            target_samples = mean_xy[None, :n, :] + rng.normal(
                size=(self.mc_samples, n, 2)
            ) * sigma_xy[None, :n, :]
