...
def _realized_ttc(ego,target_state,collision_distance_m=2.0):
    ...
    root=np.sqrt(max(0.0,disc));t1=(-b-root)/(2.0*a);t2=(-b+root)/(2.0*a);candidates=[t for t in (t1,t2) if t>=0.0]
    return min(candidates) if candidates else np.inf


def _simulate_episode(scenario,planner_name,seed,settings,link_predictor):
    rng=np.random.default_rng(seed);params=VehicleParams(dt=settings.dt);ego=np.asarray(scenario.ego_initial,float).copy();target=np.asarray(scenario.target_states,float)
    ...
    observations=[];snr=[];outage=[];ber=[];goodput=[];target_distance=[];realized_ttc=[];obstacle_clearance=[];path_length=0.0;no_candidate=0;collision=False;previous=ego[:2].copy();steps=min(len(target)-1,scenario.steps)
    ...
    return {"scenario":scenario.name,"planner":planner_name,"seed":seed,"steps":steps,"duration_s":steps*settings.dt,"mean_snr_db":float(np.mean(snr)),"mean_outage_probability":float(np.mean(outage)),"mean_ber_model":float(np.mean(ber)),"mean_goodput_bps_model":float(np.mean(goodput)),"path_length_m":path_length,"progress_m":float(ego[0]-scenario.ego_initial[0]),"min_target_distance_m":float(np.min(target_distance)),"min_realized_ttc_s":float(np.min(realized_ttc)),"min_static_obstacle_clearance_m":float(np.min(obstacle_clearance)) if obstacle_clearance else np.inf,"collision_indicator":int(collision),"no_candidate_steps":no_candidate,"measured_optical_link":False}


def run_simulated_episode(planner_name,scenario,seed=0,settings=BenchmarkSettings(),link_predictor=None):
    """Run one reproducible simulated episode without exposing future truth to P0-P3."""
    ...
    return _simulate_episode(scenario,planner_name,int(seed),settings,link_predictor)
...
def run_benchmark(seeds=range(10),settings=BenchmarkSettings(),scenarios=None,link_predictor=None):
    ...
    for scenario in scenarios:
        for seed in seeds:
            for planner in PLANNERS:rows.append(run_simulated_episode(planner,scenario,int(seed),settings,link_predictor))
    return rows
