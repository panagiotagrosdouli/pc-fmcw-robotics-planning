...
class BenchmarkSettings:
    ...
    v4_static_viability: bool=False
    v5_time_aligned_dynamic: bool=False
...
def _prediction(history,horizon,dt,damped_lateral=False):
    ...
    if damped_lateral and len(history)>1:
        # The lateral CV slope estimated from a short noisy window is not
        # assumed to persist indefinitely. Decay it over a fixed 0.5 s time
        # constant while retaining the same least-squares current estimate.
        t=np.arange(len(history),dtype=float)*dt
...
def _simulate_episode(scenario,planner_name,seed,settings,link_predictor):
    ...
    planner_common=dict(vehicle_params=params,target_clearance=planning_target_clearance,
                        require_static_stop_viability=settings.v4_static_viability,
                        bounded_brake_steer=settings.v4_static_viability)
    planner_common['time_aligned_dynamic']=settings.v5_time_aligned_dynamic
    planner_common['require_dynamic_stop_viability']=settings.v5_dynamic_stop_viability
    if settings.v6_hierarchical_clearance:
        planner_common['physical_target_clearance']=settings.collision_distance_m
    ...
    for k in range(steps):
        ...
        if getattr(planners[planner_name],"last_margin_relaxed",False):
            margin_relaxation_steps+=1
            if first_margin_relaxation_step<0:first_margin_relaxation_step=k+1
        ...
        if result.candidate is None:
            no_candidate+=1
            if first_no_candidate_step<0:first_no_candidate_step=k+1
        # A missing candidate is still counted as a hard failure. Bounded
        # braking prevents an infeasible step from silently becoming coasting.
        control=(np.array([params.min_accel,0.0])
                 if settings.v4_static_viability and result.candidate is None
                 else _first_control(result))
        ego=step(ego,control,params);path_length+=float(np.linalg.norm(ego[:2]-previous));previous=ego[:2].copy();truth_now=target[k+1]
